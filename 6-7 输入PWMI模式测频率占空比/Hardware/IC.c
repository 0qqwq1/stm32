...
{    
    ...
    TIM_TimeBaseInitStructure.TIM_Prescaler = 200 - 1;           //PSC 标准频率fc = 72M / (PSC + 1), 这里标准频率为360000
    ...
    TIM_PWMIConfig(TIM3, &TIM_ICInitStructure);                //只支持配置通道1和通道2
...
{
    return 360000 / (TIM_GetCapture1(TIM3) + 1);               //Freq = 72M / (PSC + 1) / (ARR + 1) 这里的+1是为了凑整
}
...
{
    return (TIM_GetCapture2(TIM3) + 1) * 100 / (TIM_GetCapture1(TIM3) + 1);     //先乘后除，避免小数点后被截断导致精度严重损失，甚至归零
}
