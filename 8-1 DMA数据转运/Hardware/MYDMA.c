...
void MYDMA_Init(uint32_t AddrA, uint32_t AddrB, uint16_t Size)
{
    ...
    DMA_InitStructure.DMA_PeripheralBaseAddr = AddrA;                           //起始地址通过变量AddrA赋予
    ...
    DMA_InitStructure.DMA_MemoryBaseAddr= AddrB;                                //起始地址通过变量AddrB赋予
    ...
    DMA_InitStructure.DMA_BufferSize = Size;                                    //传输计数器的值通过变量Size赋予
    DMA_InitStructure.DMA_Mode = DMA_Mode_Normal;                               //不开启自动重装器
    DMA_InitStructure.DMA_M2M = DMA_M2M_Enable;                                 //开启软件触发
    DMA_InitStructure.DMA_Priority = DMA_Priority_High;                         //设置优先级
    DMA_Init(DMA1_Channel1, &DMA_InitStructure);
    
    //如果是硬件传输这里还要加上对应外设的DMA输出
